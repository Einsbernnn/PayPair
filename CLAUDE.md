...
- `npm run build` — `quasar build` (production). Outputs to `dist/pwa`. `vueRouterBase` is `/` (root-domain deploy on Vercel); update it if you deploy under a subpath.
...
- OAuth redirect target is `${origin}${import.meta.env.BASE_URL}auth/callback`. `BASE_URL` is `/` in this repo, so the callback is `<origin>/auth/callback` (e.g. `http://localhost:9200/auth/callback` in dev, `https://<vercel-host>/auth/callback` in production). Every origin you deploy to must be registered in Supabase's allowed redirect URLs.
...
- Route base path is `/`; still prefer `import.meta.env.BASE_URL` over hardcoded `/` when constructing absolute URLs (see `services/auth.ts`) so a future subpath deploy doesn't break.
